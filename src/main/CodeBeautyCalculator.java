...
import CAMs.Balance;
import CAMs.DCM;
import CAMs.Equilibrium;
import CAMs.OverallDensity;
import CAMs.Regularity;
import CAMs.Rhythm;
import CAMs.Sequence;
import CAMs.Simplicity;
import CAMs.Symmetry;
import utils.CodeConverter;

public class CodeBeautyCalculator {

	public static void main(String[] args) throws Exception {
		String localDir = System.getProperty("user.dir");
		String directory = localDir + "\\Project";// folder that contains the project we are studying
		String filename = localDir + "\\normalized.java";// file that contains the normalized class (after replacing
															// tabs with spaces)
		String measuresPath = localDir + "\\measures\\";// folder in which the file with the results will be stored
		String javafiles = localDir + "\\javaFiles.txt";// file in which we store the paths of all java files contained
														// in the project
		String projectName = "Test";// name of the under study project

		File dir = new File(directory);

		File listOfFiles = new File(javafiles);// file in which the paths of all java files will be stored
		FileWriter fwriter;
		File fmeasures = new File(measuresPath + projectName + "_metrics.csv");// file in which the results of the
		...
		while (freader.hasNext()) {// for each code file contained in the project
			...
			CodeConverter cc = new CodeConverter(f.getAbsolutePath(), filename);// class that replaces each tab
																				// character in the file with four
																				// spaces
			...
			// The name is the path to this file inside the project, each folder name
			// separated by '/'
			...
			while (true) {
				if (tempfile.getParentFile().getName().equals(projectName)) {// if the previous folder is the one that
																				// contains the whole project
					break;
				} else {
					measurefilename = tempfile.getParentFile().getName() + "/" + measurefilename;// stores the parent
																									// file
					tempname = tempfile.getParentFile().getAbsolutePath();// the new path is the path of the previous
																			// file
